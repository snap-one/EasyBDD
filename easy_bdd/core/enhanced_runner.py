...
class EnhancedTestRunner:
    ...
    def _execute_test(self, test: TestDefinition) -> bool:
        ...
        try:
            ...
            if hasattr(test, 'device_config') and test.device_config:
                ...
                if device_data:
                    print(f"      ✅ Device config loaded: {device_data.get('device_info', {}).get('name', 'Unknown')}")
            
            # Set test-level variables in the global config
            if hasattr(test, 'variables') and test.variables:
                test_scope = self.global_config.variable_manager.get_scope("test_variables")
        ...
        if hasattr(test, 'cleanup') and test.cleanup:
            for cleanup_step in test.cleanup:
                try:
                    ...
                    print(f"      Cleanup step failed (continuing): {cleanup_step} - {e}")
    ...
    def _execute_step_enhanced(self, step: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if service_type not in self.services:
                self.services[service_type] = self._create_service_enhanced(service_type)
    ...
    def _execute_step_on_service(self, service, step: Dict[str, Any]) -> bool:
        ...
        if hasattr(service, 'execute_action'):
            ...
            print(f"        Service {type(service).__name__} doesn't support execute_action")
            return False
    ...
    def _determine_service_type(self, step: Dict[str, Any]) -> str:
        ...
        # API actions
        if any(keyword in action for keyword in ['api', 'request', 'get', 'post', 'put', 'delete', 'patch']):
            return 'api'
        
        # Browser actions
        elif any(keyword in action for keyword in ['click', 'type', 'navigate', 'wait', 'screenshot']):
            return 'browser'
    ...
    def _execute_data_driven_test(self, test: TestDefinition) -> bool:
        ...
        print(f"    Running {len(data_sets)} data iterations with enhanced variables...")
        ...
        for i, data_set in enumerate(data_sets, 1):
            ...
            iteration_scope = self.global_config.variable_manager.get_scope("runtime_data")
