...
class VariableManager:
    ...
    def substitute_variables(self, text: str, additional_vars: Optional[Dict] = None) -> str:
    ...
    def substitute_recursive(self, data: Any, additional_vars: Optional[Dict] = None) -> Any:
        """Recursively substitute variables in data structures"""
        if isinstance(data, str):
            return self.substitute_variables(data, additional_vars)
        elif isinstance(data, dict):
            return {k: self.substitute_recursive(v, additional_vars) for k, v in data.items()}
...
class APIConfigManager:
    ...
    def store_token(self, device_id: str, token_data: Dict[str, Any]):
        ...
        self.variable_manager.set_variable(f'auth_token_{device_id}', token_data.get('token'))
        self.variable_manager.set_variable(f'auth_expires_{device_id}', token_data.get('expires_at'))
...
class GlobalConfigManager:
    ...
    def _flatten_device_config(self, device_data: Dict[str, Any], device_name: str) -> Dict[str, Any]:
