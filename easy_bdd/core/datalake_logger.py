...
class DatalakeLogger:
    ...
    def error_hint(self, param1: str, param2: str) -> Optional[str]:
        ...
        try:
            for col1, col2, col3 in self.error_hint_subs:
                ...
                if col1 and (
                    col1.lower() not in param1.lower()
                    if ignore_case
                    else col1 not in param1
                ):
                    continue

                # Check second column
                match2 = not col2 or (
                    col2.lower() in param2.lower() if ignore_case else col2 in param2
                )
                if match2:
                    try:
                        hint = eval(col3)
    ...
    def gpt_hint(self, param1: str, param2: str) -> Optional[str]:
        ...
        try:
            ...
            messages = [
                ...
                {"role": "user", "content": f"{param1}: {param2}"},
            ]
    ...
    def datalake_post(
        ...
        if self.post_results:
            response = requests.request(
                "PUT", url=url, headers=headers, data=datalake_json
            )
            ...
            if response.status_code != 200:
                ...
                if self.logger:
                    self.logger.exception(
                        f"Datalake's API returns {response.status_code=}: {response.text}"
                    )
    ...
    def func_finishing_log(self, response: Any = None, nested: bool = False):
        """
    ...
    def wrapper(self, func):
        ...
        def my_inner(*args, **kwargs):
            ...
            self.func_starting_log(
                request=request, request_display=request_display, nested=nested
            )
...
def get_logger(
    artifact_path: str = "reports/artifacts", post_results: bool = True
) -> DatalakeLogger:
    """
    ...
    if _logger_instance is None:
        _logger_instance = DatalakeLogger(
            artifact_path=artifact_path, post_results=post_results
        )
