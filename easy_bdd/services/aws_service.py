...
class AWSService:
    ...
    @classmethod
    def configure_global_credentials(cls, access_key_id: str = None, 
    ...
    def _resolve_credentials(self, access_key_id: str = None, 
                            secret_access_key: str = None, 
                            region: str = None) -> Tuple[str, str, str]:
        """
        ...
        resolved_key = access_key_id or self._global_config.get('access_key_id') or os.environ.get('AWS_ACCESS_KEY_ID')
        resolved_secret = secret_access_key or self._global_config.get('secret_access_key') or os.environ.get('AWS_SECRET_ACCESS_KEY')
        resolved_region = region or self._global_config.get('region') or os.environ.get('AWS_DEFAULT_REGION', 'us-east-1')
    ...
    def _get_s3_clients(self, bucket_name: str, access_key_id: str = None,
                       secret_access_key: str = None, region: str = None):
        """
        Get or create S3 clients for the specified bucket.
        ...
        Args:
            ...
            region: AWS Region
        """
        # Only recreate if bucket or credentials changed
        if (self._bucket_name != bucket_name or 
            self._s3_client is None or 
            self._s3_resource is None):
            
            key, secret, reg = self._resolve_credentials(access_key_id, secret_access_key, region)
    ...
    def list_firmware_files(self, bucket_name: str,
        ...
        try:
            ...
            for obj in objects:
                ...
                if version_pattern and not re.search(version_pattern, obj.key):
                    continue
                ...
                if specific_version:
                    versions = specific_version if isinstance(specific_version, list) else [specific_version]
                ...
                if cloudfront_url:
                    if cloudfront_filename_only:
                        ...
                        cf_url = s3_url.replace(f"{bucket_name}.s3.amazonaws.com", cloudfront_url)
    ...
    def _sort_urls_by_version(self, urls: List[str], bucket_name: str, 
                             cloudfront_url: str = None) -> List[str]:
        """
    ...
    def get_latest_firmware(self, bucket_name: str,
                           folder_prefix: str = None,
                           filename_pattern: str = None,
                           version_pattern: str = None,
                           ...
                           store_url_as: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            # Get objects
            if folder_prefix:
                objects = list(self._current_bucket.objects.filter(Prefix=folder_prefix))
            ...
            if version_pattern:
                ...
                version_regex = r'([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9]+)?)'
            
            match = re.search(version_regex, latest_file)
    ...
    def upload_file(self, bucket_name: str,
        ...
        try:
            ...
            if make_public:
                self._s3_client.put_object_acl(ACL='public-read', Bucket=bucket_name, Key=s3_key)
    ...
    def delete_folder(self, bucket_name: str,
                     folder_prefix: str,
                     access_key_id: str = None,
                     secret_access_key: str = None,
