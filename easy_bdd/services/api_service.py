...
class APIAuthManager:
    ...
    def _authenticate_bearer_token(self, device_id: str,
        ...
        if 'credentials' in config:
            ...
            auth_endpoint = config.get('endpoint') or config.get('auth_endpoint', '/auth/login')
            ...
            print(f"    🔍 Using new device format - Retrieved username: '{username}', password: '{password}'")
        ...
        try:
            ...
            print(f"    📝 Auth payload: {username_field}='{username}', {password_field}='***'")
    ...
    def _authenticate_basic_auth(self, device_id: str,
                                 config: Dict[str, Any]) -> str:
    ...
    def _authenticate_api_key(self, device_id: str,
                              config: Dict[str, Any]) -> str:
    ...
    def _authenticate_oauth2(self, device_id: str,
                             config: Dict[str, Any]) -> str:
    ...
    def get_headers(self, device_id: str, force_refresh: bool = False) -> Dict[str, str]:
...
class APIService:
    ...
    def request(self, method: str, url: str, device_id: str = "default",
        ...
        timeout = kwargs.pop('timeout',
                             auth_config.get('timeout', self.default_timeout))
        verify = kwargs.pop('verify',
                            auth_config.get('verify_ssl', self.session.verify))
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                auth_headers = self.auth_manager.get_headers(device_id, force_refresh=True)
    ...
    def get(self, url: str, device_id: str = "default",
            **kwargs) -> requests.Response:
        """Make GET request"""
        return self.request('GET', url, device_id, **kwargs)
    
    def post(self, url: str, device_id: str = "default",
             **kwargs) -> requests.Response:
        """Make POST request"""
        return self.request('POST', url, device_id, **kwargs)
    
    def put(self, url: str, device_id: str = "default",
            **kwargs) -> requests.Response:
        """Make PUT request"""
        return self.request('PUT', url, device_id, **kwargs)
    
    def delete(self, url: str, device_id: str = "default",
               **kwargs) -> requests.Response:
        """Make DELETE request"""
        return self.request('DELETE', url, device_id, **kwargs)
    
    def patch(self, url: str, device_id: str = "default",
              **kwargs) -> requests.Response:
        """Make PATCH request"""
