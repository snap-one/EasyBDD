...
class JSONRPCWebSocketService:
    ...
    async def _handle_message(self, message: str):
        ...
        try:
            ...
            if "id" in data:
                ...
                if msg_id in self.pending_requests:
                    ...
                    if request.method == "dsStartDeviceUpdates":
                        if response.result and "updates" in response.result:
                            self.device_online = response.result["updates"].get(
                                "connected", False
                            )
                ...
                print(
                    f"    📩 Response received [{received_at}]: {data.get('id', 'unknown')}"
                )
    ...
    async def send_request(
        ...
        request = JSONRPCRequest(method=method, id=msg_id, params=params or {})
    ...
    async def set_time_settings(
        ...
        """
        params = {"deviceId": self.device_id, "version": 2, "name": timezone_name}
        ...
        response = await self.send_request(method="dxSetTimeSettings", params=params)
    ...
    async def set_status_update_frequency(self, frequency: int) -> bool:
        ...
        response = await self.send_request(
            ...
            params={"deviceId": self.device_id, "version": 0, "frequency": frequency},
        )
    ...
    async def set_cloud_server_url(self, url: str, port: int) -> bool:
        ...
        response = await self.send_request(
            ...
            params={"deviceId": self.device_id, "version": 0, "url": url, "port": port},
        )
    ...
    async def disable_cloud(self) -> bool:
        ...
        response = await self.send_request(
            method="dxDisableCloud", params={"deviceId": self.device_id, "version": 0}
        )
    ...
    async def update_firmware(self, firmware_url: str) -> bool:
        ...
        response = await self.send_request(
            ...
            params={"deviceId": self.device_id, "version": 0, "url": firmware_url},
        )
    ...
    async def find_device_by_serial(self, serial_num: str) -> Optional[Dict[str, Any]]:
        ...
        response = await self.send_request(
            method="dsFindDeviceBySerialNum", params={"serialNum": serial_num}
        )
