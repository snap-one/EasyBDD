...
class LiveRecorder:
    ...
    def parse_playwright_line(self, line: str) -> Dict[str, Any]:
        ...
        if match := re.match(r'goto\(["\']([^"\']*)["\']', line):
            return {"action": "Open browser", "url": match.group(1)}

        # get_by_role(role, name=name).click()
        if match := re.match(
            r'get_by_role\(["\'](\w+)["\'],?\s*name\s*=\s*["\']([^"\']*)["\'].*?\)\.click\(',
            line,
        ):
            return {
        ...
        if match := re.match(
            r'get_by_role\(["\'](\w+)["\'],?\s*name\s*=\s*["\']([^"\']*)["\'].*?\)\.fill\(["\']([^"\']*)["\']',
            line,
        ):
            return {
        ...
        if match := re.match(
            r'get_by_role\(["\'](\w+)["\'],?\s*name\s*=\s*["\']([^"\']*)["\']', line
        ):
        ...
        if match := re.search(
            r'locator\((["\'])(.+?)\1\)\.select_option\((["\'])(.+?)\3', line
        ):
            selector = match.group(2).replace('\\"', '"').replace("\\'", "'")
            value = match.group(4).replace('\\"', '"').replace("\\'", "'")
            return {
        ...
        if "screenshot" in line:
            return {"action": "Take screenshot", "name": "recorded_screenshot"}
...
class InteractiveRecorder:
    ...
    def start(self, url: str = None, output: str = None):
        ...
        if not output:
            output = input(
                "Output file (default: tests/cases/recorded_test.yaml): "
            ).strip()
            if not output:
