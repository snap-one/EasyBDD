...
class EasyBDDApp {
    ...
    async loadTests() {
        try {
            ...
            console.log('Tests loaded:', data.tests?.length || 0, 'tests');
            
            this.allTests = data.tests || []; // Store all tests
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 hover:shadow-md transition-shadow">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex-1">
                        ...
                        <p class="text-gray-600 dark:text-gray-300 text-sm mb-3">${test.description || 'No description available'}</p>
                    ...
                    <div class="flex items-center space-x-2 ml-4">
                        <button onclick="app.runTest('${test.filename}')" 
                                class="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors text-sm">
                            <i class="fas fa-play mr-2"></i>Run
                        </button>
                        <button onclick="app.editTest('${test.filename}')" 
    ...
    async runTest(filename) {
        try {
            const response = await fetch(`${this.API_BASE}/tests/run`, {
                ...
                body: JSON.stringify({
                    test_path: `tests/cases/${filename}`,
                    tags: [],
                    headless: true
                })
            });
            ...
            try {
                ...
                if (data.status === 'completed' || data.status === 'failed') {
                    this.showNotification(
                        `Test ${data.status}: ${data.output || 'No output'}`, 
    ...
    async exportResults() {
        ...
        try {
            ...
            const response = await fetch(`${this.API_BASE}/tests/results/export?format=${format}`);
            ...
            if (format === 'json') {
                ...
                // For CSV and XML, the response is already a file download
    ...
    async editTest(filename) {
        try {
            ...
            const response = await fetch(`${this.API_BASE}/tests/${filename.replace('.yaml', '')}`);
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            ...
            this.setupSimpleEditor(data.content, filename);
            
            this.showNotification(`Loaded test: ${filename}`, 'success');
        } catch (error) {
