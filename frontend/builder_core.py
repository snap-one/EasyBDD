"""Storage-agnostic core of the Easy BDD test builder.

Everything here describes and manipulates the case/step model (action
catalog, the CaseModel/StepNode step tree, serialization to the runner's
YAML dot-notation, validation, and reverse-parsing an existing case body
back into a model) without knowing or caring where a case is actually
stored. `frontend/testrail_builder.py` (TestRail-backed) and
`frontend/local_builder.py` (local-filesystem-backed) both import from
this module so the two apps share one implementation of "what a case is"
and only differ in how a case gets read/written.
"""
...
def _classify_role(title: str) -> str:
    """Map a case title's prefix (e.g. 'Feature: Login') to its role key.

    Pure string matching against CASE_PREFIXES — works the same whether the
    title came from a TestRail case or a local file's derived title.
    """
