...
python -m easybdd run tests/templates/device_onboarding.yaml --device $DEVICE_NAME

# Step 3: Run security audit
echo "🔒 Running security audit..."
python -m easybdd run tests/templates/security_audit.yaml --device $DEVICE_NAME

# Step 4: Generate test report
echo "📊 Generating device profile..."
python -m easybdd generate-profile --device $DEVICE_NAME --output "reports/device_profiles/"
