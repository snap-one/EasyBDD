...
class GherkinGenerator:
    ...
    def _convert_step_to_gherkin(
        self, step: TestStep, default_keyword: str = "And"
    ) -> str:
    ...
    def _is_verification_step(self, step: TestStep) -> bool:
        ...
        verify_keywords = ["verify", "check", "assert", "should", "contains", "expect"]

        action_lower = step.action.lower()
        return any(keyword in action_lower for keyword in verify_keywords)

    def generate_multiple_features(
        self, tests: List[TestDefinition], output_dir: Path
    ) -> List[Path]:
